#!/bin/bash
# Sleep benchmark script with one process per device
...
echo "=== Sleep Benchmark Parameters (Multiprocess) ==="
...
baseline=$(python bench_sleep_multiprocess.py -c $COUNT -u $MICROSECONDS -d 1 -s 1 2>&1 | grep "Completed" | sed 's/.*in \([0-9.]*\) seconds/\1/')
...
for devices in "${DEVICES[@]}"; do
    for streams in "${STREAMS[@]}"; do
        ...
        elapsed=$(python bench_sleep_multiprocess.py -c $COUNT -u $MICROSECONDS -d $devices -s $streams 2>&1 | grep "Completed" | sed 's/.*in \([0-9.]*\) seconds/\1/')
        if [ -n "$elapsed" ]; then
