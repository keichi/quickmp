...
NB_MODULE(_quickmp, m) {
    ...
    m.def(
        "abjoin",
        [](const_pyarr_t T1, const_pyarr_t T2, size_t m) {
            size_t n1 = T1.shape(0);
            size_t n2 = T2.shape(0);
            std::vector<double> P(n1 - m + 1);

            quickmp::abjoin(T1.data(), T2.data(), P.data(), n1, n2, m);
    ...
    static int dummy = 0;
    m.attr("_cleanup") = nb::capsule(&dummy, [](void *) noexcept { quickmp::finalize(); });
}
